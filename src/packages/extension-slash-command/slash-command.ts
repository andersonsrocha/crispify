...
type SlashCommandProps = { leftMenu?: boolean };
    ...
    return [
      Suggestion({
        ...
        allow: ({ state, range }) => {
          ...
          const isRootDepth = $from.depth === (this.options.leftMenu ? 2 : 1);
        ...
        command: ({ editor, props }: { editor: Editor; props: Generic }) => {
        ...
        items: ({ query }: { query: string }) => {
          const withFilteredCommands = OPTIONS.map((group) => ({
            ...group,
